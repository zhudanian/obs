    ...
    $(function () {
        ...
        $('#pagination').bootstrapPaginator({
            ...
            pageUrl: function (type, page, current) {
                let bookTypeId = $('#bookTypeId').val();
                console.log(bookTypeId);
                return '${pageContext.request.contextPath}/front/bookstore/queryByBookTypeId?bookTypeId=' + bookTypeId + '&pageNum=' + page;
            },
...
<div class="col-sm-12 row">
    <h1>${bookType.bookTypeName}</h1>
    <input type="hidden" id="bookTypeId" value="${bookType.bookTypeId}">
    <hr/>
</div>
    ...
    <c:forEach items="${bookList.list}" var="book">
        <div class="card-deck" style="display: inline-block">
            <div class="card col-sm-2">
                <input type="hidden" class="hbi" value="${book.bookId}">
                <img class="card-img-top"
                     src="${pageContext.request.contextPath}/front/bookstore/showPic?image=${book.bookImage}"
                     alt="${book.bookImage}"
                     width="175px" height="200px">
                <div class="card-body">
                    <h4 class="card-title">《${book.bookName}》</h4>
                    <label>作者 ：${book.bookAuthor}</label>
                    <span class="user clearfix pull-right" style="margin-top: 20px;
                font-size: large;color: #f8bf0f">￥${book.bookPrice}</span>
                </div>
            </div>
        </div>
    </c:forEach>
